...
export class NewResultComponent {
  ...
  fetchExams() {
    this.subscription3 = this.academicsService.getAllExams().subscribe(
      (data) => {
        this.exams = data;
      }
    );
  ...
  fetchStudents() {
    this.subscription4 = this.teacherService.getAllStudents().subscribe(
      (data) => {
        this.students = data;
      }
    );
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (this.editId) {
        this.academicsService.getResults(this.editId).subscribe(
          (details) => {
  ...
  submitForm() {
    if (this.mode === 'edit') {
      ...
      this.subscription5 = this.academicsService.editResults(this.registerForm.value).subscribe(
        (response) => {
      ...
      this.subscription6 = this.academicsService.createResults(this.registerForm.value).subscribe(
        (response) => {
          console.log(response);
          this.resetForm();
